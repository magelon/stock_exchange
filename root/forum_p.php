...
//Need to determine.
$q="select count(thread_id) as count_t from threads where user_id={$_SESSION['user_id']}";
...
$q = "SELECT thread_id, subject FROM threads  where user_id={$_SESSION['user_id']} order by thread_id limit $start,$dispaly";
$r = mysqli_query($dbc, $q);
...
//print the thread link and edit link
  ...
  if($current_page !=1){
  echo'<li><a href="forum_p.php?s='.($start-$dispaly).'&p='.$pages.'">Previous</a></li>';
  }
  //Make all the numbered pages:
  for ($i=1;$i<=$pages;$i++){

  if($i!=$current_page){
  echo '<li><a href="forum_p.php?s='.(($dispaly*($i-1))).'&p='.$pages.'">'.$i.'</a></li>';
  }else{
  echo '<li class="active"><a >'.$i.'</a>'.'</li>';
 	 ...
 	 echo '<li><a href="forum_p.php?s='.($start+$dispaly).'&p='.$pages.'">Next</a></li>';
 	 }
 	 echo '
 	 <ul>
 	 <nav>';
